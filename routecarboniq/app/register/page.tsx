      ...
      <main className="relative z-10 mx-auto flex min-h-screen w-full max-w-md items-center justify-center">
        <section className="w-full rounded-[2.5rem] border border-slate-200/60 bg-white/70 p-10 shadow-xl shadow-slate-300/50 backdrop-blur-2xl">
          ...
          <header className="mb-4">
            ...
            <h1 className="mt-8 text-3xl font-bold tracking-tight text-slate-800">Create Account</h1>
          </header>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-white/50 py-4 pl-12 pr-4 outline-none focus:border-emerald-500"
                onChange={(e) => setName(e.target.value)}
              />
            </div>
            <div className="relative">
               <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
               <input type="email" placeholder="Email" required className="w-full rounded-2xl border border-slate-200 bg-white/50 py-4 pl-12 pr-4 outline-none focus:border-emerald-500" onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="relative">
               <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
               <input type="password" placeholder="Password" required className="w-full rounded-2xl border border-slate-200 bg-white/50 py-4 pl-12 pr-4 outline-none focus:border-emerald-500" onChange={(e) => setPassword(e.target.value)} />
            </div>

            <button type="submit" className="w-full rounded-2xl bg-slate-900 py-4 font-bold text-white transition-transform hover:scale-[1.02] active:scale-95 shadow-lg">
              Register
            </button>
          </form>
         
          <button onClick={() => router.back()} className="mt-8 w-full text-center text-sm font-medium text-slate-400 hover:text-slate-600">
            Back to Login
          </button>
        </section>
      </main>
